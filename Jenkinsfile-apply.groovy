            ...
            steps {
                withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-credentials']]) {
                    withEnv([
                        ...
                        terraform apply -auto-approve -var="environment=%TF_VAR_environment%" -var="region=%TF_VAR_region%" -var-file="envs/%TF_VAR_environment%/terraform.tfvars"
                        """
                    }
                }
            }
    ...
    post {
        success {
            slackSend(
                ...
                color: 'good',
                message: "✅ Terraform apply succeeded for *${params.ENVIRONMENT}* in *${params.AWS_REGION}*.",
        ...
        failure {
            slackSend(
                channel: '#devops-alerts',
                color: 'danger',
                message: "❌ Terraform apply failed for *${params.ENVIRONMENT}* in *${params.AWS_REGION}*.",
                tokenCredentialId: 'slack-token-id'
            )
        }
    }
